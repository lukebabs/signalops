...
The mobile suite uses the same protected QA identity and failure-only artifact policy as the subscriber smoke. The current committed slice runs at 375px, 390px, and 430px phone widths and verifies production login plus Dashboard, Watchlists, Assets, Sector Rotation, Opportunities, Syncratic Intelligence, Pricing, Dashboard-to-Syncratic handoff, Assets mobile card drilldown, Opportunities mobile drilldown, SAF compatibility route drilldown, and SRI ETF progression/makeup drilldown with no 404, watchlist-context error, or page-level horizontal overflow. The primary Signal Assurance operational entry point is now MarketOps Tools for privileged users. The read-only Keycloak enrollment smoke separately verifies the Create Account path reaches registration without submitting data.

The mobile gated-route control is covered by the named subscription-enforcement canary. On 2026-09-01, the canary passed at a 390x844 mobile viewport: Explorer was denied Value Intelligence with `402 subscription_feature_required`, Sector Rotation remained available, temporary Professional state unlocked Value Intelligence, Signal Assurance remained gated for Professional, and tenant-local Institutional/admin access remained valid. The deployment-agent wrapper restored enforcement off and the pilot Explorer state after verification.
