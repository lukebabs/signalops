...
- `deploy/postgres/subscriber_project_roles.sql` creates six inert NOLOGIN group roles: a migration owner, gateway, catalog sync, global EOD, Options demand, and Options capture. It intentionally grants no access to current MarketOps tables at bootstrap; S1 migration `000091` adds only `SELECT` on `marketops_asset_universe` for the catalog-sync role, which is required to seed the read-only compatibility mapping. Production secret management must attach a distinct workload login to exactly one applicable group role; the current shared superuser credential is not eligible for subscriber-private paths.
