...
| Provider credentials, database credentials, and signing/configuration secrets | OpenBao for Kubernetes deployments; current host runtime secret files for Compose until cutover | Recover through the OpenBao/secret-management runbook. Never place credentials in database backup exports, audit records, or this repository. |
