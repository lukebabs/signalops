...
| Provider credentials, database credentials, and signing/configuration secrets | OpenBao HA for Kubernetes deployments; current host runtime secret files for Compose until cutover | Recover through the OpenBao HA secret-management runbook. Never place credentials in database backup exports, audit records, or this repository. OpenBao seal/unseal or auto-unseal, audit logs, and snapshot/restore evidence are production dependencies. |
