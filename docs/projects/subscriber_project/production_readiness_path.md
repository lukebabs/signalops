...
| Kubernetes production app/data migration | Prepared / not cut over | Production app, route, and data overlays are source-ready and server-side dry-run verified. Runtime generation now targets Kubernetes service DNS by default, and database bootstrap passwords are governed through OpenBao data-plane injection rather than Kubernetes Secret password references. Remaining gates are production OpenBao runtime provisioning, production DB restore/replication, authenticated route validation, and named traffic authority transfer. Evidence: [K8S production app/data migration package — 2026-09-13](k8s_production_app_data_migration_package_2026-09-13.md). |
