...
| Mobile subscriber UX | Expanded automated smoke passed / sprint open | `scripts/run_subscriber_mobile_ui_smoke.sh` validates production login and core subscriber routes at 375px, 390px, and 430px phone widths: Dashboard, Watchlists, Assets, Sector Rotation, Signal Assurance, Syncratic Intelligence, Dashboard-to-Syncratic handoff, Assets mobile card drilldown, SAF mobile drilldown, and SRI ETF progression/makeup drilldown. The broader PR-5 Opportunities drilldown, enrollment, subscription-pricing, and gated-route checks remain open. Latest result: `15 passed in 127.35s`. |
