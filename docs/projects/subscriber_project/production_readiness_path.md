...
Signal-Connect K8S migration has started with a source-ready, replicas-zero staging scaffold for the two concrete Go Connect workers: persister and outbox. The manifest guard is verified and now appears in the production-readiness report as `k8s_signal_connect_manifest_guard=verified`. The private GHCR worker image is published as `ghcr.io/syncratic-inc/signalops-connect-k8s-worker:347dc8d20d07` and `staging`. Live ingestion shadow remains pending until the image is pull-smoked from `signalops-connect`, the manifests are applied in-cluster, and a bounded scale-to-one shadow smoke validates broker/database behavior before scaling back to zero.
