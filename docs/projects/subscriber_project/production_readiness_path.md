...
| Kubernetes production app/data migration | Data runtime ready / not cut over | Production app, route, and data overlays are source-ready and server-side dry-run verified. OpenBao production app/data runtime provisioning is closed, and all four K8s production database pods are running on retained Longhorn PVCs with ClusterIP-only services. A dry-run-only Docker-to-K8s replication readiness tool is source-controlled; current execution is pending because the Docker source DB containers must be online and the actual copy needs a named approval. Remaining gates are production DB restore/replication, authenticated route validation, capacity/load validation, and named traffic authority transfer. Evidence: [K8S production app/data migration package — 2026-09-13](k8s_production_app_data_migration_package_2026-09-13.md). |
