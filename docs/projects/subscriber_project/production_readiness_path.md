...
| Service mesh / ingress | Mesh-2 unauthenticated route parity closed; Mesh-3 auth blocked | Istio control-plane readiness passed and unauthenticated SignalOps staging route parity passed through Playwright. Authenticated Keycloak parity is blocked before redirect validation because OIDC discovery returned HTTP 503/WAF challenge to fresh smoke traffic. Evidence: [Mesh-1](k8s_mesh1_istio_readiness_2026-09-13.md), [Mesh-2](k8s_mesh2_signalops_staging_route_parity_2026-09-13.md), and [Mesh-3 blocker](k8s_mesh3_keycloak_staging_route_blocker_2026-09-13.md). |
...
Signal-Connect K8S migration has started with a source-ready, replicas-zero staging scaffold for the two concrete Go Connect workers: persister and outbox. The manifest guard is verified and now appears in the production-readiness report as `k8s_signal_connect_manifest_guard=verified`. The private GHCR worker image is published as `ghcr.io/syncratic-inc/signalops-connect-k8s-worker:347dc8d20d07` and `staging`, the `signalops-connect` namespace pull smoke passed, the dormant scaffold is applied in-cluster with both Deployments at `0/0`, and OpenBao placeholder runtime access is verified with cross-plane denial. Live ingestion shadow remains pending until approved non-production `.svc` runtime values are installed and a bounded scale-to-one shadow smoke validates broker/database behavior before scaling back to zero.
