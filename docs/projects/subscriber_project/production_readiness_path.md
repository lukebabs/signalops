...
| Kubernetes capacity/load | Partial / headroom blocked | Staging gateway route/runtime smoke passed with 0% errors and low p95 latency across health, readiness, and synthetic signed checkout webhooks. The cluster headroom verifier is blocked because current CPU requests exceed allocatable CPU on the single node; production authority transfer requires added capacity or right-sized requests. Capacity attribution is now repeatable through `scripts/report_k8s_capacity_requests.sh`: the main pressure comes from Syncratic platform/runtime namespaces, smoke/rehearsal namespaces, Longhorn, and Istio, while SignalOps staging workloads remain scaled to zero outside bounded smokes. Evidence: [K8S capacity/load validation — 2026-09-13](k8s_capacity_load_validation_2026-09-13.md) and [K8S capacity remediation plan — 2026-09-13](k8s_capacity_remediation_plan_2026-09-13.md). |
