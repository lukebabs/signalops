...
| Service mesh / ingress | Mesh-2 unauthenticated route parity closed | Istio control-plane readiness passed on 2026-09-13, and `signalops-staging.syncratic.co` now has a staging-only HTTPRoute through `istio-system/public-ingress`. Playwright verified SPA routing plus `/healthz` and `/readyz` through the Istio route, with production cutover false and app pods scaled back to zero. Evidence: [Mesh-1 Istio readiness evidence](k8s_mesh1_istio_readiness_2026-09-13.md) and [Mesh-2 SignalOps staging route parity](k8s_mesh2_signalops_staging_route_parity_2026-09-13.md). |
