...
The Subscriber Project is **controlled paid-pilot ready with first real paid activation evidence**, moving toward full external production readiness.
...
Infrastructure planning has resumed with a source-derived Kubernetes workload inventory and a non-applied base scaffold for namespaces, service accounts, NetworkPolicies, and OpenBao-backed secret projection. OpenBao is now the selected Kubernetes secret-management plane. OpenBao HA is a resilience hardening target, not a production blocker: production can proceed with a documented single-node OpenBao operating posture when backups/restore, seal/unseal procedure, audit logging, per-plane Kubernetes auth roles, and recovery runbooks are proven. The read-only Kubernetes surface check is `scripts/verify_openbao_ha_readiness.sh`; authenticated OpenBao policy/seal/audit validation remains a separate hardening gate. The executable staging path is now documented in [Kubernetes/OpenBao staging cutover gate](k8s_openbao_staging_cutover_gate.md). On 2026-09-12 the verifier reached the cluster and confirmed only 1 OpenBao server pod was ready; that is now recorded as a resilience gap rather than a production blocker, provided backup/restore and recovery controls are current. The single-node OpenBao path is approved for continued staging and can support initial production planning while additional nodes/true HA are prepared. K8S-1 non-secret foundation scaffolding is applied and verified with zero workload pods. K8S-2 staging web/gateway manifests are rendered, server-dry-run validated, and apply mechanics were exercised; the OpenBao app staging role/path is verified with placeholder-only values and cross-plane denial evidence. A narrow app-to-OpenBao egress policy and HTTPS injector override were added after live staging evidence. Staging images are now published to private GHCR packages under `syncratic-inc`, and the cluster successfully pulled them through the refreshed `signalops-ghcr-pull` credential. The K8S-2 web runtime/probe path is verified using a Kubernetes-native nginx ConfigMap and in-container localhost probes. K8S-2 gateway runtime readiness is also verified under the approved non-production single-node OpenBao staging exception: OpenBao injected non-production runtime values, cross-plane denial remained intact, gateway `/healthz` and `/readyz` passed inside the pod, and staging workloads scaled back to zero. K8S-2 app shell/proxy parity is now closed through a bounded Playwright port-forward smoke: staging web served the SignalOps SPA, proxied gateway `/healthz` and `/readyz`, enforced four narrow staging NetworkPolicies, and scaled back to zero. Authenticated staging UX parity remains a separate gate because localhost cannot satisfy the live Keycloak production callback redirect. Docker Compose remains the live production authority until production-grade OpenBao recovery controls, CA trust, authenticated K8s app/data-plane parity, scheduler parity, rollback gates, and the executable K8S production cutover parity report pass. The parity report is defined in [K8S production cutover parity plan](k8s_production_cutover_parity_plan.md). K8S-3 has started with a source-ready suspended MarketOps CronJob scaffold and a dedicated job-runner image target. The suspended CronJob apply/list smoke passed in-cluster with five suspended CronJobs and zero Jobs/Pods created; no Kubernetes MarketOps schedule is active yet. Job-runner image publication to private GHCR is now verified, `signalops-marketops` can pull the image, and placeholder-only OpenBao MarketOps staging role/path access is verified with app-plane denial. A guarded MarketOps OpenBao runtime writer and one-shot non-provider dry-run Job harness are now source-ready; live execution is blocked until `/etc/signalops/openbao-signalops-marketops-staging-runtime.env` contains approved non-production Kubernetes `.svc` database URLs and can be read by the operator context. The job-runner image was republished after the dry-run entrypoint change and the `signalops-marketops` namespace pull smoke passed again against the refreshed private `staging` tag. The one-shot K8S-3 non-provider dry-run Job gate first passed against the non-production runtime-smoke database using OpenBao injection and the private GHCR job-runner image. DB-backed scheduler-completion parity is closed: the Kubernetes job runner records `runner=kubernetes` status/run rows and the bounded dry-run verified `succeeded|kubernetes|0|true`. The next gate also passed against dedicated non-production MarketOps primary and temporal database services in `signalops-data`, with OpenBao runtime routing and scheduler-status parity verified. The current committed job-runner image is published and pull-smoke verified as `ghcr.io/syncratic-inc/signalops-marketops-k8s-job-runner:6488950e98df`. Production-grade OpenBao CA trust is now closed for the SignalOps staging app and MarketOps worker injection paths: workloads use `signalops-openbao-ca` plus `/vault/tls/ca.crt`, and verifiers reject `tls-skip-verify`. The K8S-3 no-provider scheduler mechanics gate is now closed: one staged CronJob was unsuspended through the Kubernetes controller, created exactly one dry-run Job, resuspended immediately, wrote DB-backed scheduler parity evidence, and cleaned up with provider polling and production cutover disabled. The first provider-enabled K8S schedule smoke is also closed under named approval: the staged FMP annual CronJob executed one AAPL provider call with no retries, wrote one correlated annual evidence record, recorded scheduler success through the dedicated staging DB, resuspended immediately, and left no Jobs behind.

Production readiness is still constrained by production database restore/replication into K8s retained PVCs, explicit production traffic authority transfer, capacity/load validation, tax/invoice evidence review, and cleanup of known historical test artifacts. Production OpenBao app/data runtime provisioning is now closed. The Kubernetes work now has a source-controlled verifier, `scripts/verify_k8s_production_cutover_readiness.sh`, that reports the remaining cutover gates without moving traffic. Current dedicated MarketOps backup/restore re-verification passed on 2026-09-12. Stripe Customer Portal, refund intake, and first real paid-flow activation evidence are now retained. The core operational-consistency loop for the tenant-local pilot scope has passed natural post-close acceptance after the MarketOps database decoupling. The September 2026 outage reconciliation is closed with an accepted provider-evidence gap for unrecoverable missed options-chain source evidence. Mesh-1 is now closed: Istio base/istiod are deployed, `GatewayClass/istio` is accepted, `istio-system/public-ingress` is programmed, and SignalOps namespaces remain unenrolled. Mesh-2 unauthenticated route parity is also closed: `signalops-staging.syncratic.co` routes through Istio to staging web/gateway during bounded smokes, with app pods scaled back to zero afterward. Stripe webhook route parity is now closed through the same Istio staging route: an invalid signature was rejected, a signed synthetic `checkout.session.completed` event reconciled through the staging MarketOps database, Stripe was not called, production DNS/traffic did not move, and workloads scaled back to zero. Authenticated Keycloak parity through the mesh route is closed for staging; OIDC discovery and browser PKCE callback validation passed through the Istio route.
...
| Mobile subscriber UX | Closed for configured QA identities | `scripts/run_subscriber_mobile_ui_smoke.sh` validates production login and core subscriber routes at 375px, 390px, and 430px phone widths: Dashboard, Watchlists, Assets, Sector Rotation, Opportunities, Signal Assurance, Syncratic Intelligence, Pricing, Dashboard-to-Syncratic handoff, Assets mobile card drilldown, Opportunities mobile drilldown, SAF mobile drilldown, and SRI ETF progression/makeup drilldown. Read-only enrollment smoke also passed. The named mobile gated-route subscription-enforcement canary passed and restored production state. Latest refreshed evidence on 2026-09-12: mobile `24 passed in 47.90s`; Keycloak enrollment entrypoint `1 passed in 0.95s`; authenticated B2C active resolver `1 passed in 1.21s`; Stripe pricing readiness `1 passed in 1.98s`. Pricing now presents webhook-authoritative activation language on mobile and the enrollment checkpoint when the user arrives from the enrollment gate. |
...
| Subscription/access controls | Ready for configured QA identities / first paid activation closed | PR-2 closed tenant isolation, private-list owner projection, tier-enforcement canary, restoration, and Subscription Administration governance-surface browser evidence. B2C users remain in `tenant-local`; self-enrolled subjects without an effective subscription resolve to `subscription_missing` and route to Pricing with `source_feature=enrollment`. Pricing reads the configured Stripe product catalog, displays governed human-readable prices instead of raw Stripe Price IDs, explains that redirect return alone does not grant access, and shows that activation is webhook-authoritative. Checkout-start is operational, and the first live Explorer monthly payment reconciled through a signed webhook into `explorer active` with B2C browser state `marketops_ready`. Admin-governed refund intake is now live: users can request refunds, admins triage/record disposition, and actual refund execution remains in Stripe Dashboard. Stripe Customer Portal self-service is implemented for active Stripe-backed subscriptions, with webhook-authoritative entitlement updates retained. |
| Backup/restore | Ready / current | Fresh constrained `backup-run` completed successfully on 2026-09-12 UTC. Isolated restore rehearsal passed for both `marketops-primary` and `marketops-temporal`; operations monitor subsequently exited successfully. Evidence: [PR-3 backup/restore refresh evidence — 2026-09-12](pr3_backup_restore_refresh_evidence_2026-09-12.md). |
| Service mesh / ingress | Mesh route parity closed; production cutover still gated | Istio control-plane readiness passed, unauthenticated SignalOps staging route parity passed, authenticated Keycloak/PKCE route parity passed, Mesh-4 ingress/DNS rollback planning is verified, and Stripe webhook route parity is closed through a synthetic signed checkout event. Evidence: [Mesh-1](k8s_mesh1_istio_readiness_2026-09-13.md), [Mesh-2](k8s_mesh2_signalops_staging_route_parity_2026-09-13.md), [Mesh-4](k8s_mesh4_ingress_dns_cutover_plan_2026-09-13.md), and [Stripe route parity](k8s_stripe_webhook_route_parity_2026-09-13.md). |
| Kubernetes production app/data migration | Data runtime ready / not cut over | Production app, route, and data overlays are source-ready and server-side dry-run verified. OpenBao production app/data runtime provisioning is closed, and all four K8s production database pods are running on retained Longhorn PVCs with ClusterIP-only services. A Docker-to-K8s MarketOps-only replication readiness tool is source-controlled and dry-run verified; actual MarketOps copy needs a corrected named approval. Shared SignalOps database migration is explicitly separate from the MarketOps cutover path. Remaining gates are production DB restore/replication, authenticated route validation, capacity/load validation, and named traffic authority transfer. Evidence: [K8S production app/data migration package — 2026-09-13](k8s_production_app_data_migration_package_2026-09-13.md). |
| Kubernetes capacity/load | Partial / single-node resilience gap | Staging gateway route/runtime smoke passed with 0% errors and low p95 latency across health, readiness, and synthetic signed checkout webhooks. The cluster headroom verifier now reports `status=ok` after active workload pressure dropped to 67.28% CPU requests, but the cluster remains single-node. Capacity attribution is repeatable through `scripts/report_k8s_capacity_requests.sh`; worker-node addition remains the preferred production resilience remediation and has a prepared runbook requiring at least one practical 8 vCPU worker. Evidence: [K8S capacity/load validation — 2026-09-13](k8s_capacity_load_validation_2026-09-13.md), [K8S capacity remediation plan — 2026-09-13](k8s_capacity_remediation_plan_2026-09-13.md), and [K8S worker node addition runbook — 2026-09-13](k8s_worker_node_addition_runbook_2026-09-13.md). |
...
| Outage reconciliation | Closed with accepted provider-evidence gap | September 2026 recovery exposed fixed `/tmp` lock ownership drift in scheduled scripts and a historical EEOM projection defect. Runtime locks now use a shared helper, notification recording accepts governed non-binary job statuses, and migration `000174_subscriber_global_eeom_historical_projection` restores session-aware EEOM projection. Warm EOD recovered as governed degraded `995/1000`; unavailable missed options-chain evidence remains visible as `provider_evidence_missing` and is not fabricated. Closure evidence is recorded in [September 2026 outage reconciliation closure](outage_reconciliation_closure_2026-09-12.md). |
...
11. **Mobile subscriber readiness**
   ...
   - Acceptance:
     ...
     - Dashboard, Watchlists, Assets, Market State, Value Intelligence, Distressed Opportunity Intelligence, Earnings Opportunity Intelligence, Opportunities, SRI, Syncratic, Pricing, enrollment flows, and SAF compatibility drill-downs have no blocking mobile usability defects. The primary SAF operational workbench is privileged under Tools.
...
Status: first slice deployed to production on 2026-09-05.
...
5. **Operations health in Admin** — keep expanding Admin-visible freshness/job status so routine validation does not require shell access or HAR handoffs. Sep 11 outage recovery added a constrained dated deployment-agent action, `marketops-outage-reconcile:YYYY-MM-DD`, for missed completed trading-day recovery. Current Sep 2 slice exposes `MarketOps operations monitor` as a constrained Admin run-now action, validates it through Playwright, and verifies the reprovisioned deployment-agent `scheduler-status` includes the operations-monitor timer/service.
...
Production readiness impact: scheduler parity moves from pending to closed. Remaining migration blockers are capacity/load validation and explicit production authority transfer. The Istio/Gateway API ingress/DNS rollback plan and Stripe webhook K8S route parity are now documented as verified non-cutover gates; they do not authorize traffic movement.


### K8S Signal-Connect shadow scaffold — 2026-09-13

Signal-Connect K8S migration has closed the staging shadow gate for the two concrete Go Connect workers: persister and outbox. The manifest guard is verified and appears in the production-readiness report as `k8s_signal_connect_manifest_guard=verified`. The private GHCR worker image is published as `ghcr.io/syncratic-inc/signalops-connect-k8s-worker:347dc8d20d07` and `staging`; the namespace pull smoke passed; non-production `.svc` runtime values were installed through OpenBao with cross-plane denial; a staging Redpanda broker smoke passed; and the bounded sequential scale-to-one Connect shadow smoke validated broker/database behavior before scaling back to zero.
