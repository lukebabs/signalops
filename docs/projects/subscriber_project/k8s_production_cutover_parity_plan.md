...
This closes the source/manifest guard, private GHCR publication, namespace pull smoke, and dormant in-cluster apply for the Connect worker shape. The Connect worker image is published as `ghcr.io/syncratic-inc/signalops-connect-k8s-worker:347dc8d20d07` and `staging`; `signalops-connect-persister` and `signalops-connect-outbox` are applied with `0/0` replicas. `pending_signal_connect_ingestion_shadow=true` remains correct until one bounded scale-to-one shadow smoke proves broker/database behavior and the workloads scale back to zero. Evidence is tracked in [K8S-4 Signal-Connect shadow scaffold — 2026-09-13](k8s4_signal_connect_shadow_scaffold_2026-09-13.md).
