...
This closes the source/manifest guard for the Connect worker shape and the private GHCR publication step. The Connect worker image is published as `ghcr.io/syncratic-inc/signalops-connect-k8s-worker:347dc8d20d07` and `staging`. `pending_signal_connect_ingestion_shadow=true` remains correct until the image is pull-smoked from `signalops-connect`, the scaffold is applied, one bounded scale-to-one shadow smoke proves broker/database behavior, and the workloads scale back to zero. Evidence is tracked in [K8S-4 Signal-Connect shadow scaffold — 2026-09-13](k8s4_signal_connect_shadow_scaffold_2026-09-13.md).
