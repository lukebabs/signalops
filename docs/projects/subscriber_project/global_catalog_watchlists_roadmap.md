...
### Approved aggregate hot-selector boundary — 2026-08-16

The explicitly approved constrained selector boundary is implemented in migration `000145_subscriber_global_hot_intraday_selector_reader`. `signalops_subscriber_global_selector` is `NOLOGIN`, `NOINHERIT`, non-superuser, and has no role memberships. Its only purpose is to own one `SECURITY DEFINER` function returning five de-identified aggregate fields: global asset ID, canonical symbol, watcher count, first-selected time, and last-selected time. The global EOD worker may execute that function and read the security-barrier view; it has no `SELECT` privilege on subscriber watchlist-context tables.

With the tenant-local analyst’s explicit selection persisted on the dedicated MarketOps primary, immutable shadow run `subhotshadow-fbb86a74f80aaad1cda09034460e7c4f` passed at 2026-08-16 22:48 UTC: 125 cohort members, 125 selector members, 125 shared, zero cohort-only, and zero selector-only. The seven catalog-ineligible legacy symbols remain preserved but excluded from intraday activation. This gate does not turn on a new scheduler or central intraday writer.
