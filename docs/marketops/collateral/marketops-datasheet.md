...
| Strategic valuation research | Value Intelligence (Valuation Composite / VC) and Distressed Opportunity Intelligence (DOSM) | Transparent strategic rankings based on retained financial and market snapshots |
...
| Sector Rotation Intelligence | Price-led cross-sectional ranks for core sectors and industries versus SPY, QQQ, and RSP | A transparent way to choose where deeper research may be most relevant, without claiming rotation or a trade |
...
|---|---|---|---|
| Strategic | Weekly financial refresh; cached snapshots reused between refreshes | Is the available financial evidence relatively attractive, weak, or incomplete? | Value Intelligence (VC) and Distressed Opportunity Intelligence (DOSM) |
...
- **Value Intelligence (Valuation Composite / VC):** an explainable 0–10 relative-valuation score using price-to-sales, GAAP price-to-earnings, and EV/EBITDA, with a bounded peer adjustment when peer coverage is adequate.
- **Distressed Opportunity Intelligence (DOSM):** a 0–10 strategic research rank that combines Value Intelligence, operating quality, cash generation, debt profile, capital efficiency, and a bounded technical adjustment.
...
- **Sector Rotation Intelligence (SRI):** a versioned price-led cross-sectional context rank built from 5-, 20-, and 60-session ETF returns, benchmark-relative strength, momentum, and acceleration. It is explicitly not a rotation, flow, breadth, or recommendation engine.
...
- Value Intelligence and Distressed Opportunity Intelligence are currently **TTM-only**. Three-year revenue CAGR and its valuation penalty are intentionally withheld until a 16-quarter point-in-time history is available.
...
- Sector Rotation Intelligence requires 61 canonical price sessions for each primary ETF and each benchmark. Partial availability is displayed as not-ready context, never as a low rank.
