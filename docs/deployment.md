...
Deployment-cache contract: nginx serves the app shell with `Cache-Control: no-store, no-cache, must-revalidate`; build-hashed `/assets/` files are immutable for one year when present and return a true `404` when absent. The SPA performs one guarded reload for a Vite dynamic-import/preload failure, recovering an open tab that spans a deployment without a reload loop.
