...
The Administration → System workbench monitors SignalOps-owned persistent volumes only: primary PostgreSQL, TimescaleDB, and Redpanda. It deliberately excludes Docker images, writable container layers, engine logs, and unrelated host volumes.

`signalops-storage-monitor.timer` runs every fifteen minutes. Its collector mounts only the three named volumes read-only and writes one durable snapshot per store to `storage_monitor_snapshots`. It has no Docker socket or writable volume mount.
