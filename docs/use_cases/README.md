...
- `cyberops/`: CyberOps firewall evidence and lifecycle-noise control where `app_id=cyberops`, `domain=security`, and `use_case=cyberops`.

CyberOps local development uses `tenant_id=tenant-local`; a production deployment uses the tenant ID of the organization it serves. Adding the app profile does not create a new tenant boundary.
