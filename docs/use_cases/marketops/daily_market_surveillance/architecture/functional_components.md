...
| Bounded explicit-cohort state materialization | Build deterministic state-v2 records and quality-blocked longitudinal transitions over existing equity/options/event evidence. | `signalops-marketops-state-materializer`, 50 definitions, 75 slots/39 required, explicit cohorts capped at 10, exact lineage, idempotent upserts. |
...
- G144: 50-definition/75-slot state v2, longitudinal and event features, multi-window/acceleration/regime transitions, H001 known-event invalidation, and explicit state cohorts capped at 10.
