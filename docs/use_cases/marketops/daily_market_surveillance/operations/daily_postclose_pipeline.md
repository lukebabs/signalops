...
- The normalization barrier requires at least 45 distinct same-session equity symbols before options acquisition.
- Options acquisition uses an explicit 45-symbol supported list, at most two 250-record pages and 500 transient candidates per symbol, no automatic retries, and at most seven persisted evidence contracts per symbol.
...
The five current equity provider gaps remain `2222.SR`, `005930.KS`, `000660.KS`, `601939.SS`, and `RO.SW`. Changing the supported list requires a reviewed provider or cross-listing decision; the workflow never silently remaps an asset.
