...
- Market State records all usable close-to-close returns and emits `large_completed_session_move` evidence at an inclusive absolute 3.00% threshold; it remains research-only and requires independent same-session corroboration before the final convergence queue can use it.
