...
Calls Syncratic Ask server-side with a compact, bounded non-human reasoning prompt built from the deterministic context window, then persists the generated explanation and Ask metadata onto the associated Syncratic insight. The G090 implementation uses Syncratic Ask `scope=tenant`, `k=1`, `thread_mode=off`, `include_refs=false`, `direct_reasoning=true`, `graph_enabled=false`, `kee_enabled=false`, and one `external_context.items[]` entry containing the bounded SignalOps context; no facade filters are sent. SignalOps metadata stays inside the bounded external context and local persisted metrics. The prompt explicitly instructs Syncratic to reason only from the supplied JSON context and to produce a useful MarketOps explanation instead of returning `UNKNOWN` when signals are present. The bounded external context includes compact signal details and data-quality checks; when subject mismatches are detected, the Ask prompt switches to `analysis_mode=data_quality_blocked` so Syncratic warns about unsupported evidence instead of generating market interpretation.
