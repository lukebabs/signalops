...
func TestSessionEnrollmentAllowsAuthenticatedUserWithoutExistingAccess(t *testing.T) {
	...
	router := NewRouter(RouterConfig{Auth: fixture.authCfg, AccessRepository: repo, SubscriberB2CTenantID: "tenant-local"})
...
func TestSessionEnrollmentProvisionsB2CExplorerAccessAfterEmailVerification(t *testing.T) {
	...
	if repo.lastUpsert.TenantID != "tenant-local" || repo.lastUpsert.Subject != "user-123" || repo.lastUpsert.AppID != "marketops" || repo.lastUpsert.Permission != "read" {
...
func TestSessionEnrollmentDoesNotSelfProvisionNonB2CTenant(t *testing.T) {
	fixture := newTestAuthFixture(t)
	repo := &accessManagementTestRepository{}
	router := NewRouter(RouterConfig{Auth: fixture.authCfg, AccessRepository: repo, SubscriberB2CTenantID: "tenant-pilot-b"})
...
func TestSessionEnrollmentDoesNotRequireSubscriptionWhenEnforcementDisabled(t *testing.T) {
	...
	router := NewRouter(RouterConfig{Auth: fixture.authCfg, AccessRepository: accessRepo, SubscriberSubscriptionRepository: subscriptionRepo, SubscriberB2CTenantID: "tenant-local"})
	token := fixture.token(t, map[string]any{"email_verified": true})
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, withBearer(httptest.NewRequest(http.MethodGet, "/v1/session/enrollment", nil), token))
	if recorder.Code != http.StatusOK || !strings.Contains(recorder.Body.String(), `"state":"marketops_ready"`) {
...
func TestSessionEnrollmentRequiresSubscriptionForB2CWhenEnforcementEnabled(t *testing.T) {
	...
	router := NewRouter(RouterConfig{Auth: fixture.authCfg, AccessRepository: accessRepo, SubscriberSubscriptionRepository: subscriptionRepo, SubscriberSubscriptionAdministrationRepository: adminRepo, SubscriberSubscriptionsEnabled: true, SubscriberB2CTenantID: "tenant-local"})
	token := fixture.token(t, map[string]any{"tenant_id": "tenant-local", "email_verified": true})
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, withBearer(httptest.NewRequest(http.MethodGet, "/v1/session/enrollment", nil), token))
	if recorder.Code != http.StatusOK || !strings.Contains(recorder.Body.String(), `"state":"subscription_missing"`) {
...
func TestSessionEnrollmentCanAutoActivateExplorerWhenExplicitlyEnabled(t *testing.T) {
	...
	subscriptionRepo := &subscriberSubscriptionAPIFake{err: storage.ErrNotFound}
	adminRepo := &subscriberSubscriptionAdministrationFake{}
	router := NewRouter(RouterConfig{Auth: fixture.authCfg, AccessRepository: accessRepo, SubscriberSubscriptionRepository: subscriptionRepo, SubscriberSubscriptionAdministrationRepository: adminRepo, SubscriberSubscriptionsEnabled: true, SubscriberB2CTenantID: "tenant-local", SubscriberB2CAutoActivateExplorer: true})
	token := fixture.token(t, map[string]any{"tenant_id": "tenant-local", "email_verified": true})
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, withBearer(httptest.NewRequest(http.MethodGet, "/v1/session/enrollment", nil), token))
	if adminRepo.subject.TenantID != "tenant-local" || adminRepo.subject.Subject != "user-123" || adminRepo.subject.ProductKey != "explorer" || adminRepo.subject.Status != storage.SubscriberSubscriptionActive {
...
func TestSessionEnrollmentExistingB2CSubscriptionDoesNotReEnroll(t *testing.T) {
	fixture := newTestAuthFixture(t)
	accessRepo := &accessManagementTestRepository{subjectAccess: []storage.TenantUserAccessRecord{{TenantID: "tenant-local", Subject: "user-123", AppID: "marketops", Permission: "read"}}}
	subscriptionRepo := &subscriberSubscriptionAPIFake{record: storage.SubscriberEffectiveSubscriptionRecord{TenantID: "tenant-local", Subject: "user-123", SubscriptionID: "sub-existing", Status: storage.SubscriberSubscriptionActive, Source: "subject", Product: storage.SubscriberSubscriptionProductRecord{ProductKey: "explorer", BillingScope: "subject", DisplayName: "Explorer", Active: true, FeaturePolicyJSON: []byte(`{"dashboard":true}`), LimitPolicyJSON: []byte(`{"private_watchlists":3}`), Revision: 1}}}
	adminRepo := &subscriberSubscriptionAdministrationFake{}
	router := NewRouter(RouterConfig{Auth: fixture.authCfg, AccessRepository: accessRepo, SubscriberSubscriptionRepository: subscriptionRepo, SubscriberSubscriptionAdministrationRepository: adminRepo, SubscriberSubscriptionsEnabled: true, SubscriberB2CTenantID: "tenant-local"})
	token := fixture.token(t, map[string]any{"tenant_id": "tenant-local", "email_verified": true})
	recorder := httptest.NewRecorder()
...
func TestSessionEnrollmentExistingB2CSubscriptionBypassesMutationThrottle(t *testing.T) {
	...
	accessRepo := &accessManagementTestRepository{subjectAccess: []storage.TenantUserAccessRecord{{TenantID: "tenant-local", Subject: "user-123", AppID: "marketops", Permission: "read"}}}
	subscriptionRepo := &subscriberSubscriptionAPIFake{record: storage.SubscriberEffectiveSubscriptionRecord{TenantID: "tenant-local", Subject: "user-123", SubscriptionID: "sub-existing", Status: storage.SubscriberSubscriptionActive, Source: "subject", Product: storage.SubscriberSubscriptionProductRecord{ProductKey: "explorer", BillingScope: "subject", DisplayName: "Explorer", Active: true, FeaturePolicyJSON: []byte(`{"dashboard":true}`), LimitPolicyJSON: []byte(`{"private_watchlists":3}`), Revision: 1}}}
	router := NewRouter(RouterConfig{Auth: fixture.authCfg, AccessRepository: accessRepo, SubscriberSubscriptionRepository: subscriptionRepo, SubscriberSubscriptionsEnabled: true, SubscriberB2CTenantID: "tenant-local"})
	token := fixture.token(t, map[string]any{"tenant_id": "tenant-local", "email_verified": true})
	for i := 0; i < 20; i++ {
		recorder := httptest.NewRecorder()
