...
func scheduledJobStatuses() []map[string]any {
  ...
  dir := os.Getenv("SIGNALOPS_SCHEDULE_STATUS_DIR"); if dir == "" { dir = "/var/lib/signalops/scheduled-jobs" }
  out := make([]map[string]any, 0, len(jobs))
  for _, job := range jobs {
    row := map[string]any{"job_id": job.ID, "label": job.Label, "schedule": job.Schedule, "timezone": job.Timezone, "status": "pending"}
    if raw, err := os.ReadFile(filepath.Join(dir, job.ID+".json")); err == nil { var recorded map[string]any; if json.Unmarshal(raw, &recorded) == nil { for k, v := range recorded { row[k] = v } } }
    out = append(out, row)
  }
  return out
}
