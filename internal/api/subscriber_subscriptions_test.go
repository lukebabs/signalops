...
func TestSubscriberSubscriptionProductsExposePublicCommercialCatalog(t *testing.T) {
	...
	store := &subscriberSubscriptionAPIFake{products: []storage.SubscriberSubscriptionProductRecord{{ProductKey: "professional", BillingScope: "subject", DisplayName: "Professional", TrialDays: 7, StripeProductID: "prod_test", StripeMonthlyPriceID: "price_monthly", StripeAnnualPriceID: "price_annual", MonthlyDisplayPrice: "$99/mo", AnnualDisplayPrice: "$999/yr", FeaturePolicyJSON: []byte(`{"value_intelligence":true}`), LimitPolicyJSON: []byte(`{"private_watchlists":20}`), Revision: 1}}}
	...
	if recorder.Code != http.StatusOK || !strings.Contains(recorder.Body.String(), `"professional"`) || !strings.Contains(recorder.Body.String(), `"stripe_monthly_price_id":"price_monthly"`) || !strings.Contains(recorder.Body.String(), `"monthly_display_price":"$99/mo"`) || !strings.Contains(recorder.Body.String(), `"checkout_enabled":false`) {
