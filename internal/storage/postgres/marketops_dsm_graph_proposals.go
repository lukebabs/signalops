...
INSERT INTO marketops_dsm_graph_proposals (
 ...
 node_id, from_node, relationship, to_node, labels, properties, raw_candidate, status
...
ON CONFLICT (proposal_id) DO UPDATE SET
 ...
 labels=EXCLUDED.labels, properties=EXCLUDED.properties, raw_candidate=EXCLUDED.raw_candidate, updated_at=now()`,
		...
		record.SourceID, record.SourceAdapter, record.Dataset, record.ArtifactID, record.SignalID, record.SignalType,
		record.DetectorID, record.Severity, record.Confidence, record.EventIDs, record.SubjectSymbol, record.CandidateType,
		record.NodeID, record.FromNode, record.Relationship, record.ToNode, record.Labels, jsonOrEmpty(record.PropertiesJSON),
		jsonOrEmpty(record.RawCandidate), graphProposalStatusOrDefault(record.Status))
	if err != nil {
...
ORDER BY updated_at DESC LIMIT $11`, strings.TrimSpace(filter.TenantID), strings.TrimSpace(filter.AppID), strings.TrimSpace(filter.Domain), strings.TrimSpace(filter.UseCase),
		...
		strings.TrimSpace(filter.CandidateType), strings.TrimSpace(filter.Status), clampLimit(filter.Limit))
	if err != nil {
...
SELECT proposal_id, tenant_id, app_id, domain, use_case, source_id, source_adapter, dataset, artifact_id,
 signal_id, signal_type, detector_id, severity, confidence, COALESCE(array_to_json(event_ids), '[]'::json)::text,
...
func scanMarketOpsDSMGraphProposal(scanner marketOpsDSMGraphProposalScanner) (storage.MarketOpsDSMGraphProposalRecord, error) {
	...
	if err := scanner.Scan(&record.ProposalID, &record.TenantID, &record.AppID, &record.Domain, &record.UseCase,
		&record.SourceID, &record.SourceAdapter, &record.Dataset, &record.ArtifactID, &record.SignalID, &record.SignalType,
		&record.DetectorID, &record.Severity, &record.Confidence, &eventIDsJSON, &record.SubjectSymbol, &record.CandidateType,
