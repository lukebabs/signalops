...
// ListSubscriberGlobalEROCResults reads only the parity-approved platform
// projection. The Gateway supplies symbols authorized by the selected list.
func (r *Repository) ListSubscriberGlobalEROCResults(ctx context.Context, symbols []string, limit int) ([]storage.MarketOpsValuationResultRecord, error) {
	if len(symbols) == 0 {
...
FROM subscriber_gateway_global_eroc_results
WHERE upper(symbol) = ANY($1)
ORDER BY session_date DESC, score DESC, symbol ASC
LIMIT $2`, pqArray(symbols), valuationLimit(limit))
	if err != nil {
		return nil, fmt.Errorf("list subscriber global EROC results: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ResultID, &item.SnapshotID, &item.TenantID, &item.Symbol, &item.SessionDate, &item.AlgorithmID, &item.ModelVersion, &item.Score, &item.FairValue, &item.Classification, &item.Confidence, &item.ConfidenceLabel, &item.EvaluationStatus, &item.Eligible, &item.ResultJSON, &item.CreatedAt); err != nil {
			return nil, fmt.Errorf("scan subscriber global EROC result: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate subscriber global EROC results: %w", err)
	}
	return out, nil
}
