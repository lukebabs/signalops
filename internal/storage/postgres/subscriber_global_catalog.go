...
func upsertSubscriberGlobalAsset(ctx context.Context, tx *sql.Tx, id string, x subscriberCatalogSourceAsset, observedAt time.Time) (bool, error) {
	...
	var inserted bool
	err := tx.QueryRowContext(ctx, `INSERT INTO subscriber_global_assets (global_asset_id,source_id,provider_symbol,canonical_symbol,company_name,asset_type,exchange,sector,industry,eligibility_status,reference_effective_at,reference_provenance,first_seen_at,last_seen_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'discovered',$10,$11::jsonb,$12,$12) ON CONFLICT (source_id,provider_symbol) DO UPDATE SET canonical_symbol=EXCLUDED.canonical_symbol,company_name=EXCLUDED.company_name,asset_type=EXCLUDED.asset_type,exchange=EXCLUDED.exchange,sector=CASE WHEN COALESCE(subscriber_global_assets.reference_provenance->>'authority','')='fmp_profile' THEN subscriber_global_assets.sector ELSE EXCLUDED.sector END,industry=CASE WHEN COALESCE(subscriber_global_assets.reference_provenance->>'authority','')='fmp_profile' THEN subscriber_global_assets.industry ELSE EXCLUDED.industry END,reference_effective_at=CASE WHEN COALESCE(subscriber_global_assets.reference_provenance->>'authority','')='fmp_profile' THEN subscriber_global_assets.reference_effective_at ELSE EXCLUDED.reference_effective_at END,reference_provenance=CASE WHEN COALESCE(subscriber_global_assets.reference_provenance->>'authority','')='fmp_profile' THEN subscriber_global_assets.reference_provenance ELSE EXCLUDED.reference_provenance END,last_seen_at=EXCLUDED.last_seen_at,updated_at=now() RETURNING (xmax=0)`, id, x.SourceID, x.Ticker, x.Ticker, x.Company, x.AssetType, x.Exchange, x.Sector, x.Industry, x.UpdatedAt, string(provenance), observedAt).Scan(&inserted)
