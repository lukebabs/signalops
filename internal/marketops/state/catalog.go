...
var g137FeatureSpecs = []featureSpec{
	...
	{Key: "range_position_252d", Domain: "risk_reward", Title: "252-session price-range position", Unit: "percent", RequiredInputs: []string{"equity.high", "equity.low", "equity.close"}, Calculation: map[string]any{"operator": "close_position_in_high_low_range", "lookback_sessions": 252}},
	{Key: "volume_ratio_10d", Domain: "risk_reward", Title: "Volume ratio to 10-session mean", Unit: "ratio", RequiredInputs: []string{"equity.volume"}, Calculation: map[string]any{"operator": "ratio_to_mean", "lookback_sessions": 10}},
	{Key: "distance_sma_50_pct", Domain: "risk_reward", Title: "Distance from 50-session SMA", Unit: "percent", RequiredInputs: []string{"equity.close"}, Calculation: map[string]any{"operator": "distance_from_sma", "lookback_sessions": 50}},
	{Key: "distance_sma_200_pct", Domain: "risk_reward", Title: "Distance from 200-session SMA", Unit: "percent", RequiredInputs: []string{"equity.close"}, Calculation: map[string]any{"operator": "distance_from_sma", "lookback_sessions": 200}},
	{Key: "sma_50_slope_20d_pct", Domain: "risk_reward", Title: "50-session SMA 20-session slope", Unit: "percent", RequiredInputs: []string{"equity.close"}, Calculation: map[string]any{"operator": "sma_change", "sma_sessions": 50, "lookback_sessions": 20}},
	...
	{Key: "put_call_volume_ratio_10d_deviation_pct", Domain: "risk_reward", Title: "Put/call volume deviation from 10-session mean", Unit: "percent", RequiredInputs: []string{"put_call_volume_ratio"}, Calculation: map[string]any{"operator": "ratio_deviation_from_mean", "lookback_sessions": 10}, QualityPolicy: map[string]any{"missing_is_not_zero": true}},
...
var g144FeatureSpecs = []featureSpec{
	...
	{Key: "earnings_window_state", Domain: "market_event_context", Title: "Known earnings-window state", RequiredInputs: []string{"market_event_calendar.earnings_date", "market_event_calendar.known_at"}, Calculation: map[string]any{"operator": "classified_earnings_window", "pre_event_days": 10, "post_event_days": 2}},
}
