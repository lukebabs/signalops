...
func Evaluate(in Input) Result {
	...
	if in.CallVolume > 0 {
		...
		if r.TotalOptionVolume >= 1000 {
			if putCall < .30 { r.OptionsFlowExtreme = "call_volume_extreme" } else if putCall > 1.20 { r.OptionsFlowExtreme = "put_volume_extreme" }
		}
	}
...
func driftDirection(closes []float64) Direction {
	if closes[len(closes)-1] < closes[0] { return DirectionBullish }
	if closes[len(closes)-1] > closes[0] { return DirectionBearish }
	return DirectionNone
}
func consecutive(c []float64, down bool) int { n := 0; for i := len(c)-1; i > 0; i-- { if (down && c[i] < c[i-1]) || (!down && c[i] > c[i-1]) { n++ } else { break } }; return n }
func directionalWindow(c []float64, down bool) (int, int) { bestN, bestW := 0, 0; for _, w := range []int{5,6,7} { n := 0; for i := len(c)-w; i < len(c); i++ { if (down && c[i] < c[i-1]) || (!down && c[i] > c[i-1]) { n++ } }; if n*5 >= w*4 && (n > bestN || n == bestN && w > bestW) { bestN, bestW = n, w } }; return bestN, bestW }
func directionalRate(c []float64, down bool) float64 { best := 0.0; for _, w := range []int{5,6,7} { n := 0; for i := len(c)-w; i < len(c); i++ { if (down && c[i] < c[i-1]) || (!down && c[i] > c[i-1]) { n++ } }; best = math.Max(best, float64(n)/float64(w)) }; return best }
func persistenceScore(streak int, rate float64) float64 { return clamp(math.Max(math.Min(float64(streak)/4, 1), rate) * 100) }
func extensionPercent(c []float64, down bool) float64 { current := c[len(c)-1]; extreme := current; for _, close := range c { if down && close > extreme { extreme = close }; if !down && close < extreme { extreme = close } }; if extreme <= 0 { return 0 }; return math.Abs(current-extreme) / extreme * 100 }
func meanAbsoluteDailyMove(c []float64) float64 { values := make([]float64, 0, len(c)-1); for i := 1; i < len(c); i++ { if c[i-1] > 0 { values = append(values, math.Abs(c[i]/c[i-1]-1)*100) } }; return mean(values) }
...
func stance(direction Direction, score float64) float64 { if direction == DirectionBullish { return score }; if direction == DirectionBearish { return -score }; return 0 }
func outcome(direction Direction, complete bool) string { prefix := "incomplete"; if complete { prefix = "complete" }; if direction == DirectionBullish { return prefix+"_bullish_reversal_review" }; if direction == DirectionBearish { return prefix+"_bearish_reversal_review" }; return "unresolved_not_ranked" }
func tier(score float64, complete bool) string { if complete { return "CONFIRMED" }; if score >= 75 { return "STRONG" }; if score >= 50 { return "DEVELOPING" }; if score >= 25 { return "EMERGING" }; return "LOW" }
func clamp(value float64) float64 { if value < 0 { return 0 }; if value > 100 { return 100 }; return value }
func mean(values []float64) float64 { if len(values) == 0 { return 0 }; total := 0.0; for _, value := range values { total += value }; return total/float64(len(values)) }
