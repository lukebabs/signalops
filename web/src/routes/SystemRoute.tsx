...
export function SystemRoute() {
  ...
  const taskManager = useMarketOpsTaskManager(TENANT_ID);
  const retryTask = useMutateMarketOpsTaskRetry(TENANT_ID);
  ...
  const operationsDataFreshness = marketOpsOperationsHealthData?.data_freshness ?? [];
...
<h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">MarketOps Task Manager</h2><div className="overflow-x-auto rounded border border-gray-200 bg-white dark:border-slate-800 dark:bg-slate-950"><table className="min-w-full divide-y divide-gray-200 text-xs dark:divide-slate-800"><thead className="bg-gray-50 text-left text-gray-500 dark:bg-slate-900 dark:text-slate-300"><tr><th className="px-2 py-1">Job</th><th className="px-2 py-1">State</th><th className="px-2 py-1">Dependencies</th><th className="px-2 py-1">Reason</th><th className="px-2 py-1">Action</th></tr></thead><tbody className="divide-y divide-gray-100 dark:divide-slate-800">{(taskManager.data?.tasks ?? []).map((task:any)=><tr key={task.job_id}><td className="px-2 py-2 font-medium text-gray-900 dark:text-slate-100">{task.label}<div className="font-mono text-[10px] text-gray-500 dark:text-slate-400">{task.job_id}</div></td><td className="px-2 py-2"><StatusBadge status={task.state}/></td><td className="px-2 py-2 text-gray-600 dark:text-slate-300">{(task.dependencies ?? []).join(", ") || "None"}</td><td className="px-2 py-2 text-gray-600 dark:text-slate-300">{task.reason || "—"}</td><td className="px-2 py-2"><button type="button" disabled={!task.retry_eligible || retryTask.isPending} onClick={()=>retryTask.mutate(task.job_id)} className="rounded border border-brand-300 px-2 py-1 text-[11px] text-brand-700 disabled:cursor-not-allowed disabled:opacity-40 dark:border-brand-700 dark:text-brand-200">{retryTask.isPending && retryTask.variables===task.job_id ? "Retrying…" : task.retry_eligible ? "Retry" : "No action"}</button></td></tr>)}</tbody></table>{taskManager.isError ? <div className="p-2 text-xs text-red-700 dark:text-red-300">Unable to load task manager.</div> : null}</div><h2 className="text-sm font-semibold text-gray-900">MarketOps task control</h2><div className="overflow-x-auto rounded border border-gray-200 bg-white"><table className="min-w-full divide-y divide-gray-200 text-xs"><thead className="bg-gray-50 text-left text-gray-500"><tr><th className="px-2 py-1">Task</th><th className="px-2 py-1">Asset</th><th className="px-2 py-1">State</th><th className="px-2 py-1">Reason</th><th className="px-2 py-1">Retry</th></tr></thead><tbody className="divide-y divide-gray-100">{marketOpsTasks.data?.tasks.filter((task:any)=>task.status!=="succeeded").map((task:any)=><tr key={task.task_id}><td className="px-2 py-1">{task.task_type}</td><td className="px-2 py-1 font-mono">{task.symbol||"—"}</td><td className="px-2 py-1"><StatusBadge status={task.status}/></td><td className="px-2 py-1 text-gray-600">{task.failure_class||task.error_message||"—"}</td><td className="px-2 py-1 text-gray-600">{task.status==="retry_scheduled"?formatUtc(task.next_attempt_at):"—"}</td></tr>)}</tbody></table>{marketOpsTasks.data?.tasks.filter((task:any)=>task.status!=="succeeded").length===0?<div className="p-2 text-xs text-gray-500">No incomplete MarketOps tasks.</div>:null}</div>{marketOpsTasks.isError?<ErrorState error={marketOpsTasks.error}/>:null}<h2 className="text-sm font-semibold text-gray-900">Replay Operations</h2>
      <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
        <MetricTile
          label="Replay Worker"
