    ...
    <div className="space-y-3">
      ...
      {query.isLoading && !data ? (
        ...
        <>
          ...
          {data.shared_eod_coverage?.length ? <section data-testid="marketops-shared-eod-coverage" className="rounded border border-emerald-200 bg-emerald-50 p-2 text-xs text-emerald-900"><div className="font-semibold">Shared EOD coverage · {data.shared_eod_coverage.length}/{watchlist.context?.member_count ?? data.asset_count} selected assets ready</div><div className="mt-1 flex flex-wrap gap-2">{data.shared_eod_coverage.map((asset) => <span key={asset.ticker} className="rounded border border-emerald-200 bg-white px-2 py-1"><span className="font-mono font-semibold">{asset.ticker}</span> · {asset.session_date} · close {asset.close?.toLocaleString(undefined, { maximumFractionDigits: 2 }) ?? "—"}</span>)}</div><p className="mt-1 text-emerald-800">This is central completed-session evidence. Algorithm breadth remains empty until the shared algorithm pipeline has materialized its own outputs.</p></section> : null}
