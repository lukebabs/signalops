...
function GovernanceTables({ snapshot }: { snapshot: SubscriberSubscriptionAdministrationResponse | null }) {
  return <div className="grid gap-4 xl:grid-cols-2"><Table title="Subject subscriptions" headers={['Subject', 'Plan', 'Status', 'Stripe', 'Updated']} rows={(snapshot?.subject_subscriptions ?? []).map((r) => [<IdentityCell subject={r.subject} displayName={r.subject_display_name} email={r.subject_email} />, r.display_name, <StatusPill value={r.status} />, compactStripe(r.stripe_customer_id, r.stripe_subscription_id), formatDate(r.updated_at)])} empty="No subject subscriptions for this tenant." /><Table title="Institutional contracts" headers={['Tenant', 'Plan', 'Status', 'Stripe', 'Updated']} rows={(snapshot?.tenant_subscriptions ?? []).map((r) => [r.tenant_id, r.display_name, <StatusPill value={r.status} />, compactStripe(r.stripe_customer_id, r.stripe_subscription_id), formatDate(r.updated_at)])} empty="No institutional contract for this tenant." /><Table title="Institutional seats" headers={['Subject', 'Assigned by', 'Role', 'Status']} rows={(snapshot?.seats ?? []).map((r) => [<IdentityCell subject={r.subject} displayName={r.subject_display_name} email={r.subject_email} />, <IdentityCell subject={r.assigned_by} displayName={r.assigned_by_display_name} email={r.assigned_by_email} />, r.seat_role, <StatusPill value={r.status} />])} empty="No seats for this tenant." /><Table title="Audit trail" headers={['When', 'Actor', 'Subject', 'Event']} rows={(snapshot?.audit_events ?? []).map((r) => [formatDate(r.occurred_at), <IdentityCell subject={r.actor_subject} displayName={r.actor_display_name} email={r.actor_email} />, <IdentityCell subject={r.subject} displayName={r.subject_display_name} email={r.subject_email} />, r.event_type])} empty="No subscription audit events for this tenant." /><Table title="Stripe webhook ledger" headers={['Received', 'Event', 'Status', 'Error']} rows={(snapshot?.billing_webhook_events ?? []).map((r) => [formatDate(r.received_at), r.event_type, <StatusPill value={r.processing_status} />, r.error_message || '—'])} empty="No Stripe webhook events recorded." /></div>;
}
