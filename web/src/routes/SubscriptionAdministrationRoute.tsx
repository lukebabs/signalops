...
export function SubscriptionAdministrationRoute() {
  ...
  const productBillingSubmit = (event: FormEvent) => { event.preventDefault(); void run('product-billing', async () => { await api.updateSubscriberSubscriptionProductBilling(editingProductKey, { stripe_product_id: stripeProductDraft.trim(), stripe_monthly_price_id: stripeMonthlyDraft.trim(), stripe_annual_price_id: stripeAnnualDraft.trim(), correlation_id: correlation('stripe-product') }); }); };
  ...
  return <div className="max-w-7xl space-y-4">
    ...
    <nav aria-label="Subscription administration sections" className="flex gap-2 overflow-x-auto rounded border border-gray-200 bg-white p-2">{subscriptionTabs.map((tab) => <button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)} className={`shrink-0 rounded px-3 py-2 text-left text-xs ${activeTab === tab.key ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'}`}><span className="block text-sm font-semibold">{tab.label}</span><span className={`block max-w-44 ${activeTab === tab.key ? 'text-gray-200' : 'text-gray-500'}`}>{tab.description}</span></button>)}</nav>
...
function ProductBillingMap({ products }: { products: SubscriberSubscriptionProduct[] }) {
  return <section className="rounded border border-gray-200 bg-white p-4"><h2 className="text-sm font-semibold">Stripe product map</h2><p className="mt-1 text-xs text-gray-500">Read-only billing reference for every configured tier. Select a tier in Tier settings before editing its mapping.</p><Table title="Configured Stripe products" headers={['Tier', 'Scope', 'Product', 'Monthly price', 'Annual price']} rows={products.map((product) => [product.display_name, product.billing_scope, stripeCode(product.stripe_product_id), stripeCode(product.stripe_monthly_price_id), stripeCode(product.stripe_annual_price_id)])} empty="No subscription products configured." /></section>;
}

function SubjectTables({ snapshot, compact = false }: { snapshot: SubscriberSubscriptionAdministrationResponse | null; compact?: boolean }) {
  const subjectRows = (snapshot?.subject_subscriptions ?? []).map((r) => [<IdentityCell subject={r.subject} displayName={r.subject_display_name} email={r.subject_email} />, r.display_name, <StatusPill value={r.status} />, compactStripe(r.stripe_customer_id, r.stripe_subscription_id), formatDate(r.updated_at)]);
  const contractRows = (snapshot?.tenant_subscriptions ?? []).map((r) => [r.tenant_id, r.display_name, <StatusPill value={r.status} />, compactStripe(r.stripe_customer_id, r.stripe_subscription_id), formatDate(r.updated_at)]);
  const seatRows = (snapshot?.seats ?? []).map((r) => [<IdentityCell subject={r.subject} displayName={r.subject_display_name} email={r.subject_email} />, <IdentityCell subject={r.assigned_by} displayName={r.assigned_by_display_name} email={r.assigned_by_email} />, r.seat_role, <StatusPill value={r.status} />]);
...
function AuditLogView({ snapshot, search, onSearch }: { snapshot: SubscriberSubscriptionAdministrationResponse | null; search: string; onSearch: (value: string) => void }) {
  ...
  return <section className="rounded border border-gray-200 bg-white p-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-sm font-semibold">Audit log</h2><p className="mt-1 text-xs text-gray-500">Log-proficient view for governance events. Search actor, subject, event, correlation, or after-state payload.</p></div><Field label="Search audit log" value={search} onChange={onSearch} placeholder="actor, subject, event, correlation…" /></div><Table title={`Audit trail (${records.length})`} headers={['When', 'Actor', 'Subject', 'Event', 'Correlation']} rows={records.map((r) => [formatDate(r.occurred_at), <IdentityCell subject={r.actor_subject} displayName={r.actor_display_name} email={r.actor_email} />, <IdentityCell subject={r.subject} displayName={r.subject_display_name} email={r.subject_email} />, <span className="font-mono text-[11px]">{r.event_type}</span>, <span className="break-all font-mono text-[11px]">{r.correlation_id || '—'}</span>])} empty="No subscription audit events match the search." /></section>;
}
...
function WebhookLedgerView({ snapshot, search, onSearch }: { snapshot: SubscriberSubscriptionAdministrationResponse | null; search: string; onSearch: (value: string) => void }) {
  ...
  return <section className="rounded border border-gray-200 bg-white p-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-sm font-semibold">Stripe webhook ledger</h2><p className="mt-1 text-xs text-gray-500">Operational processing evidence for signed Stripe webhook events. Search event type, status, provider event ID, or error text.</p></div><Field label="Search webhooks" value={search} onChange={onSearch} placeholder="event, status, provider ID…" /></div><Table title={`Stripe webhook ledger (${records.length})`} headers={['Received', 'Event', 'Status', 'Provider event', 'Error']} rows={records.map((r) => [formatDate(r.received_at), r.event_type, <StatusPill value={r.processing_status} />, <span className="break-all font-mono text-[11px]">{r.provider_event_id}</span>, r.error_message || '—'])} empty="No Stripe webhook events match the search." /></section>;
}
...
function IdentityCell({ subject, displayName, email }: { subject?: string; displayName?: string; email?: string }) {
  ...
  const secondary = identifier && primary !== identifier ? identifier : '';
  return <span className="block min-w-[12rem]"><span className="block font-medium text-gray-900">{primary}</span>{displayName && email && displayName !== email ? <span className="block text-[11px] text-gray-500">{displayName}</span> : null}{secondary ? <span className="block break-all font-mono text-[10px] text-gray-500">{secondary}</span> : null}</span>;
}
...
function StatusPill({ value }: { value: string }) { return <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium ${value === 'active' || value === 'trialing' || value === 'processed' ? 'bg-green-100 text-green-800' : value === 'revoked' || value === 'canceled' || value === 'inactive' || value === 'unmatched' ? 'bg-gray-100 text-gray-700' : value === 'failed' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>{value}</span>; }
function stripeCode(value?: string) { return value ? <span className="break-all font-mono text-[11px]">{value}</span> : '—'; }
function compactStripe(customer?: string, subscription?: string) { return customer || subscription ? <span className="font-mono text-[11px]">{customer || '—'}<br />{subscription || '—'}</span> : '—'; }
