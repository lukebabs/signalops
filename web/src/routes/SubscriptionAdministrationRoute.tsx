...
function Summary({ snapshot }: { snapshot: SubscriberSubscriptionAdministrationResponse | null }) {
  ...
  return <div className="grid grid-cols-2 gap-2 md:grid-cols-3 xl:grid-cols-6"><MetricTile label="Products" value={snapshot?.products.length ?? 0} /><MetricTile label="Subject subscribers" value={activeSubjects} /><MetricTile label="Institutional seats" value={activeSeats} /><MetricTile label="Stripe product maps" value={mappedStripeProducts} /><MetricTile label="Audit events" value={snapshot?.audit_events.length ?? 0} /><MetricTile label="Webhook events" value={snapshot?.billing_webhook_events.length ?? 0} /></div>;
}
...
function IdentityCell({ subject, displayName, email }: { subject?: string; displayName?: string; email?: string }) {
  ...
  return <span className="block min-w-[12rem]"><span className="block font-medium text-gray-900 dark:text-gray-100">{primary}</span>{displayName && email && displayName !== email ? <span className="block text-[11px] text-gray-500 dark:text-gray-400">{displayName}</span> : null}{secondary ? <span className="block break-all font-mono text-[10px] text-gray-500 dark:text-gray-400">{secondary}</span> : null}</span>;
}
