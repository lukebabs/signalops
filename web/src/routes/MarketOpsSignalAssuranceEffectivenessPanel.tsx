...
function DailyCohortValidationChart({ rows, windowDays }: { rows: MarketOpsSignalAssuranceDailyCohortValidation[]; windowDays: string }) {
  ...
  const latestEvaluable = [...scoped].reverse().find((row) => row.evaluated > 0);
  ...
  return <div data-testid="saf-daily-cohort-validation" className="space-y-3">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      {latest ? <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4"><div className="rounded border border-gray-200 bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800"><div className="text-gray-500 dark:text-gray-400">Latest session</div><div className="font-semibold">{latest.session_date}</div></div><div className="rounded border border-gray-200 bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800"><div className="text-gray-500 dark:text-gray-400">Cohort coverage</div><div className="font-semibold">{latest.signal_coverage}/{latest.cohort_size}</div></div><div className="rounded border border-gray-200 bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800"><div className="text-gray-500 dark:text-gray-400">Latest evaluable day</div><div className="font-semibold">{latestEvaluable?.session_date ?? 'Pending'}</div><div className="text-[11px] text-gray-500 dark:text-gray-400">{latestEvaluable ? `${latestEvaluable.directional_hits}/${latestEvaluable.evaluated} validated` : 'Awaiting next close'}</div></div><div className="rounded border border-gray-200 bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800"><div className="text-gray-500 dark:text-gray-400">Accuracy</div><div className="font-semibold">{pct(latestEvaluable?.directional_accuracy ?? undefined)}</div></div></div> : null}
    </div>
