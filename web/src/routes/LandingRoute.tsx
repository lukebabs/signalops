...
export function LandingRoute() {
  ...
  return <div className="mx-auto max-w-4xl space-y-7 py-8">
    ...
    <section className="space-y-3"><div><h2 className="text-base font-semibold text-gray-900">Your domains</h2><p className="text-sm text-gray-500">Choose a workspace available to your account.</p></div>{profiles.length === 0 ? <div className="rounded border border-gray-200 bg-white p-4 text-sm text-gray-600">No domain access has been assigned yet. Ask a SignalOps super-admin to grant MarketOps or CyberOps access.</div> : <div className="grid gap-3 md:grid-cols-2">{profiles.map((profile) => { const Icon = iconFor(profile.domains); return <a key={profile.app_id} href={profile.app_id === 'streams' ? '/streams' : profile.app_id === 'narrativeops' ? '/narrativeops' : '#'} onClick={(event) => { if (profile.app_id !== 'streams' && profile.app_id !== 'narrativeops') { event.preventDefault(); void navigate({ to: profile.default_route as never }); } }} className="group block rounded border border-gray-200 bg-white p-4 text-left transition-all hover:-translate-y-0.5 hover:border-brand-300 hover:bg-brand-50 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-2"><span className="rounded bg-brand-50 p-2 text-brand-700 group-hover:bg-white"><Icon size={18} /></span><div><div className="font-semibold text-gray-900">{profile.label}</div><span className="text-xs text-gray-500">{profile.permission === 'write' ? 'Write access' : 'Read access'}</span></div></div><ArrowRight size={17} className="mt-1 text-gray-400 group-hover:text-brand-700" /></div><p className="mt-3 text-sm leading-5 text-gray-600">{profile.landing_summary}</p></a>; })}</div>}</section>
  </div>;
}
