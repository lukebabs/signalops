...
export interface SubscriberSubjectSubscriptionAdminRecord { tenant_id:string; subject:string; subscription_id:string; product_key:string; display_name:string; status:string; trial_ends_at?:string; current_period_ends_at?:string; grace_ends_at?:string; canceled_at?:string; provisioned_by:string; correlation_id:string; created_at:string; updated_at:string; }
export interface SubscriberTenantSubscriptionAdminRecord { tenant_id:string; subscription_id:string; product_key:string; display_name:string; status:string; current_period_ends_at?:string; grace_ends_at?:string; canceled_at?:string; provisioned_by:string; correlation_id:string; created_at:string; updated_at:string; }
export interface SubscriberSubscriptionSeatAdminRecord { tenant_id:string; subject:string; tenant_subscription_id:string; seat_role:string; status:string; assigned_by:string; correlation_id:string; assigned_at:string; revoked_at?:string; }
export interface SubscriberSubscriptionAuditAdminRecord { audit_id:string; tenant_id:string; subject:string; subscription_id:string; actor_subject:string; event_type:string; before_state:Record<string, unknown>; after_state:Record<string, unknown>; correlation_id:string; occurred_at:string; }
export interface SubscriberSubscriptionAdministrationResponse { tenant_id:string; products:SubscriberSubscriptionProduct[]; subject_subscriptions:SubscriberSubjectSubscriptionAdminRecord[]; tenant_subscriptions:SubscriberTenantSubscriptionAdminRecord[]; seats:SubscriberSubscriptionSeatAdminRecord[]; audit_events:SubscriberSubscriptionAuditAdminRecord[]; }
