...
export interface MarketOpsSignalAssuranceEffectiveness { evidence_source: string; dimension: string; dimension_value: string; sample_size: number; directional_hits: number; materialized_count: number; invalidated_count: number; expired_count: number; censored_count: number; excluded_count: number; directional_accuracy?: number; accuracy_lower_bound?: number; accuracy_upper_bound?: number; materialization_rate?: number; average_return?: number; average_relative_return?: number; average_mfe?: number; average_mae?: number; exploratory: boolean; viability_state: string; viability_reasons: string[]; viability_policy_version: string; as_of: string; metric_definition_version: string; }
