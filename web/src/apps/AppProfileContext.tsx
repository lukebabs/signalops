...
import { CONSOLE_PROFILE, CYBEROPS_PROFILE, MARKETOPS_PROFILE, STREAMS_PROFILE } from './appProfiles';
...
export function AppProfileProvider({ children }: { children: ReactNode }) {
  ...
  const profiles = useMemo<Array<AppProfile & { permission?: string }>>(() => {
    if (!authEnabled) return [CONSOLE_PROFILE, MARKETOPS_PROFILE, CYBEROPS_PROFILE, STREAMS_PROFILE];
