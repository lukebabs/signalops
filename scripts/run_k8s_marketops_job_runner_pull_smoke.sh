...
POD_NAME="${SIGNALOPS_K8S_MARKETOPS_PULL_SMOKE_POD:-signalops-marketops-job-runner-pull-smoke}"
GHCR_USER="${GHCR_USER:-}"

fail() {
  echo "signalops_k8s_marketops_job_runner_pull_smoke_failed: $*" >&2
  exit 1
}
...
smoke_yaml="$(mktemp -t signalops-marketops-pull-smoke.XXXXXX.yaml)"
  ...
  containers:
    - name: pull-smoke
      ...
      command:
        - /bin/sh
        - -ec
        - echo signalops_marketops_job_runner_image_pulled
      resources:
        requests:
          cpu: 10m
          memory: 32Mi
...
[[ "$logs" == *"signalops_marketops_job_runner_image_pulled"* ]] || fail "pull smoke log marker missing"
