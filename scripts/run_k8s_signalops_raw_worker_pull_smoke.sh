...
IMAGE="${SIGNALOPS_PYTHON_WORKER_STAGING_IMAGE:-ghcr.io/syncratic-inc/signalops-python-worker:staging}"
POD_NAME="${SIGNALOPS_K8S_RAW_WORKER_PULL_SMOKE_POD:-signalops-raw-worker-pull-smoke}"
GHCR_USER="${GHCR_USER:-}"

fail() {
  echo "signalops_k8s_raw_worker_pull_smoke_failed: $*" >&2
  exit 1
}
...
smoke_yaml="$(mktemp -t signalops-raw-worker-pull-smoke.XXXXXX.yaml)"
...
metadata:
  ...
  namespace: ${NAMESPACE}
  labels:
    app.kubernetes.io/name: signalops-raw-worker
  ...
  containers:
    - name: pull-smoke
      ...
      command:
        - python
        - -c
        - print("signalops_raw_worker_image_pulled")
      resources:
        requests:
          cpu: 10m
          memory: 64Mi
...
[[ "$logs" == *"signalops_raw_worker_image_pulled"* ]] || fail "pull smoke log marker missing"
