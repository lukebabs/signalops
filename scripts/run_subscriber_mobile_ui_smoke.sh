#!/usr/bin/env bash
# Run the read-only mobile Subscriber Project browser smoke with the pilot QA identity.
...
# Admin/operator workflows are intentionally excluded from the mobile subscriber gate.
exec "$repo_dir/.venv/bin/python" -m pytest -q \
  "$repo_dir/python/tests/test_subscriber_mobile_ui_smoke.py"
