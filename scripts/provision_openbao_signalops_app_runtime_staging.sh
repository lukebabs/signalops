...
fail() {
  echo "openbao_signalops_app_runtime_staging_failed: $*" >&2
  exit 1
}
...
[[ "${SIGNALOPS_K8S_STAGING_RUNTIME_NON_PRODUCTION_APPROVED:-}" == "true" ]] || fail "set SIGNALOPS_K8S_STAGING_RUNTIME_NON_PRODUCTION_APPROVED=true in the runtime env file"
...
for key in required_urls:
    ...
    forbidden_exact = {'postgres.staging.invalid', 'timescaledb.staging.invalid', 'marketops-postgres.staging.invalid', 'marketops-timescaledb.staging.invalid'}
    forbidden_hosts = {'localhost', '127.0.0.1', 'postgres', 'timescaledb', 'marketops-postgres', 'marketops-timescaledb'}
    forbidden_fragments = ('production', 'prod-', '-prod', 'signalops-postgres-1', 'signalops-marketops-postgres-1', 'signalops-marketops-timescaledb-1')
...
lines.extend([
    ...
    f'if BAO_TOKEN="$deny_token" bao kv get {shlex.quote(os.environ["OPENBAO_KV_MOUNT"] + "/" + os.environ["APP_SECRET_PATH"])} >/tmp/signalops-openbao-deny-out 2>/tmp/signalops-openbao-deny-err; then echo "cross-plane denial failed" >&2; exit 1; fi',
])
