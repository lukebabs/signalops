#!/usr/bin/env bash
# Run the authenticated Dashboard performance guardrail.
set -Eeuo pipefail
...
"$repo_dir/.venv/bin/python" -m pytest -q "$repo_dir/python/tests/test_marketops_dashboard_performance_ui.py"
printf '%s\n' 'marketops_dashboard_performance_ui_smoke_passed'
