...
ENV_FILE="${1:-.env}"
IMAGE_NAME="${SIGNALOPS_CONNECT_K8S_WORKER_IMAGE:-ghcr.io/syncratic-inc/signalops-connect-k8s-worker}"
GHCR_USER="${GHCR_USER:-}"

fail() {
  echo "signalops_k8s_connect_worker_publish_failed: $*" >&2
  exit 1
}
...
local_tag="signalops-connect-k8s-worker:${source_tag}"

printf '%s' "$GHCR_KEY" | docker login ghcr.io -u "$GHCR_USER" --password-stdin >/dev/null
docker build --no-cache --target signalops-connect-k8s-worker -t "$local_tag" . >/dev/null
...
cat <<EOF
signalops_k8s_connect_worker_publish_verified
registry=ghcr.io
user=${GHCR_USER}
image=${IMAGE_NAME}
