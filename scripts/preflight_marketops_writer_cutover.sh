...
compare() {
  ...
  printf '%-9s %-58s source=%-10s target=%-10s' "$kind" "$table" "$sc" "$tc"
  if [[ "$sc" == "$tc" ]]; then printf ' OK\n'; else printf ' MISMATCH\n'; failures=$((failures+1)); fi
}

echo 'MarketOps writer-cutover preflight (read-only)'
...
if [[ "$failures" -gt 0 ]]; then
  echo "Preflight failed: $failures mismatch(es). No writer cutover is authorized."
  exit 4
fi
