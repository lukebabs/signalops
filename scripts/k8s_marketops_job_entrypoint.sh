...
case "$job_id" in
  ...
  marketops-task-retry)
    command_args=(
      bash
      -ec
      'session_date="${MARKETOPS_SESSION_DATE:-$(date -u -d yesterday +%F 2>/dev/null || date -u +%F)}"; symbols="$(psql "$SIGNALOPS_MARKETOPS_DATABASE_URL" -v ON_ERROR_STOP=1 -Atc "SELECT symbol FROM marketops_task_items WHERE tenant_id=\$tenant\$tenant-local\$tenant\$ AND session_date=to_date(\$session\$${session_date}\$session\$,\$format\$YYYY-MM-DD\$format\$) AND task_type=\$task\$tactical_posture\$task\$ AND status=\$status\$retry_scheduled\$status\$ AND next_attempt_at <= now() ORDER BY symbol" | paste -sd, -)"; dependency_status="$(psql "$SIGNALOPS_MARKETOPS_DATABASE_URL" -v ON_ERROR_STOP=1 -Atc "SELECT COALESCE((SELECT status FROM marketops_scheduled_job_statuses WHERE job_id='marketops-daily-postclose' LIMIT 1),'missing')")"; if [ "$dependency_status" != succeeded ]; then psql "$SIGNALOPS_MARKETOPS_DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO marketops_job_dependency_evaluations(evaluation_id,session_date,job_id,state,reason,dependencies,correlation_id) VALUES ('${run_id}-evaluation',to_date('${session_date}','YYYY-MM-DD'),'marketops-task-retry','blocked','dependency marketops-daily-postclose is ${dependency_status}','[{"job_id":"marketops-daily-postclose","status":"${dependency_status}"}]'::jsonb,'${run_id}') ON CONFLICT DO NOTHING"; echo "marketops_k8s_task_retry_blocked session=${session_date} dependency_status=${dependency_status}"; exit 0; fi; if [ -z "$symbols" ]; then psql "$SIGNALOPS_MARKETOPS_DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO marketops_job_dependency_evaluations(evaluation_id,session_date,job_id,state,reason,dependencies,correlation_id) VALUES ('${run_id}-evaluation',to_date('${session_date}','YYYY-MM-DD'),'marketops-task-retry','deferred','no due retry work','[]'::jsonb,'${run_id}') ON CONFLICT DO NOTHING"; echo "marketops_k8s_task_retry_no_due_work session=${session_date}"; exit 0; fi; psql "$SIGNALOPS_MARKETOPS_DATABASE_URL" -v ON_ERROR_STOP=1 -c "INSERT INTO marketops_job_dependency_evaluations(evaluation_id,session_date,job_id,state,reason,dependencies,correlation_id) VALUES ('${run_id}-evaluation',to_date('${session_date}','YYYY-MM-DD'),'marketops-task-retry','retryable','due retry work is eligible','[]'::jsonb,'${run_id}') ON CONFLICT DO NOTHING; INSERT INTO marketops_task_retry_decisions(decision_id,job_id,session_date,decision,reason,attempt,next_attempt_at,actor,correlation_id) VALUES ('${run_id}-decision','marketops-task-retry',to_date('${session_date}','YYYY-MM-DD'),'accepted','bounded tactical retry dispatch',1,now(),'k8s-task-manager','${run_id}') ON CONFLICT DO NOTHING"; if [ "$MARKETOPS_K8S_DRY_RUN" = true ]; then echo "marketops_k8s_task_retry_dry_run_verified session=${session_date} symbols=${symbols}"; exit 0; fi; signalops-marketops-tactical-valuation-runner --tenant-id tenant-local --universe-group all_active --session-date "$session_date" --symbols "$symbols" --max-retries 2'
    )
    ;;
  marketops-warm-eod)
    if [[ "$mode_flag" == "--dry-run" ]]; then
