#!/usr/bin/env bash
# Run the controlled Syncratic Ask browser smoke with the tenant-local QA/admin
# identity. The dotenv file is parsed as literal data; it is never executed.
set -Eeuo pipefail
...
: "${SIGNALOPS_WEB_ADMIN:?SIGNALOPS_WEB_ADMIN must identify the tenant-local Syncratic Ask QA account}"
...
exec "$repo_dir/.venv/bin/python" -m pytest -q \
  "$repo_dir/python/tests/test_syncratic_ask_ui_smoke.py"
