...
valuation_global="$(marketops_primary_psql -Atc "SELECT count(*) FROM subscriber_gateway_global_valuation_results WHERE algorithm_id IN ('signalops.algorithms.valuation_composite_v3','signalops.algorithms.distressed_opportunity_scoring_v3','signalops.algorithms.valuation_composite_v4_annual','signalops.algorithms.distressed_opportunity_scoring_v4_annual') AND session_date=DATE '$session_date'" | tr -d '[:space:]')"
