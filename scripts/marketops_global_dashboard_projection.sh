...
project_kind() {
  ...
  output="$(marketops_compose --profile subscriber-global-evidence run --rm subscriber-global-marketops-parity-manifest --execute --evidence-kinds "$kind" --session-date "$session_date" --newest-first --limit 1000 --correlation-id "postclose-global-dashboard-${session_date}")"
  printf '%s\n' "$output"
