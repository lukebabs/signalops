...
for symbol in "${symbol_list[@]}"; do
  if ! marketops_compose --profile marketops-daily run --rm marketops-options-feature-materializer --tenant-id tenant-local --symbol "$symbol" --window 10_trade_days --limit 1 --run-id "${run_prefix}-features-${symbol}"; then
...
risk_reward_start="${session_date}T00:00:00Z"
for symbol in "${symbol_list[@]}"; do
...
if ! marketops_compose --profile marketops-daily run --rm marketops-algorithm-adjudicator --tenant-id tenant-local --correlation-id "$run_prefix"; then
