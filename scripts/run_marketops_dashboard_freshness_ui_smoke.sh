#!/usr/bin/env bash
# Run the authenticated tenant-local Dashboard freshness regression.
set -Eeuo pipefail
...
"$repo_dir/.venv/bin/python" -m pytest -q "$repo_dir/python/tests/test_marketops_dashboard_freshness_ui.py"
