...
printf 'Available actions: render-cutover-env, scheduler-preflight, scheduler-intraday-run, scheduler-intraday-enable, scheduler-intraday-disable, scheduler-fmp-annual-enable, scheduler-fmp-annual-disable, marketops-docker-schedulers-disable, scheduler-run-now:<job_id>, scheduler-status, operations-monitor-install, operations-monitor-run, operations-monitor-enable, operations-monitor-disable, watch-limits-stage, shared-postgres-pgbackrest-reconcile, backup-run, restore-rehearsal-run, k8s-production-db-sources-start, k8s-production-db-replication-dry-run, k8s-staging-gateway-runtime-smoke, k8s-marketops-non-provider-dry-run, marketops-recovery-resume, marketops-outage-reconcile:YYYY-MM-DD, marketops-session-reconcile:YYYY-MM-DD, marketops-postclose-systemd-reconcile, marketops-fmp-systemd-reconcile, marketops-global-market-state-migration, subscriber-subscription-commerce-migration, subscriber-global-intraday-shadow-migration, subscriber-global-intraday-shadow-dry-run, subscriber-global-intraday-shadow-schedule-once, subscriber-global-eod-history-materialize, subscriber-qualified-warm-cohort-reconcile, marketops-fmp-annual-run, fmp-annual-entitlement-preflight, subscriber-pilot-ui-smoke, marketops-saf-projection-refresh, subscription-enforcement-canary, signalops-production-deploy, signalops-production-web-deploy, signalops-production-gateway-deploy, marketops-web-deploy, marketops-gateway-deploy, marketops-writer-cutover, scheduler-run-now:marketops-retention-governance\n'
