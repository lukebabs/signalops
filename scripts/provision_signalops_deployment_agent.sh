...
[[ -x "$source_agent" && -x "$source_renderer" && -x "$source_browser_smoke" && -x "$source_bridge" && -r "$bridge_template" ]] || {
...
printf 'Available actions: render-cutover-env, scheduler-preflight, scheduler-intraday-run, scheduler-intraday-enable, scheduler-intraday-disable, scheduler-run-now:<job_id>, scheduler-status, operations-monitor-install, operations-monitor-run, operations-monitor-enable, operations-monitor-disable, watch-limits-stage, backup-run, restore-rehearsal-run, marketops-recovery-resume, marketops-global-market-state-migration, subscriber-subscription-commerce-migration, subscriber-global-intraday-shadow-migration, subscriber-global-intraday-shadow-dry-run, subscriber-global-intraday-shadow-schedule-once, subscriber-global-eod-history-materialize, subscriber-qualified-warm-cohort-reconcile, marketops-fmp-annual-run, fmp-annual-entitlement-preflight, subscriber-pilot-ui-smoke, subscription-enforcement-canary, marketops-web-deploy\n'
