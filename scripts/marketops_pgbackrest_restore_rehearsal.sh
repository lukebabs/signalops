...
for target in "${targets[@]}"; do
  ...
  docker run --rm --user postgres -v "$config_path:/etc/pgbackrest/pgbackrest.conf:ro" -v "$volume:/var/lib/postgresql/data" "$image" pgbackrest --stanza="$stanza" --type=none --archive-mode=off restore
