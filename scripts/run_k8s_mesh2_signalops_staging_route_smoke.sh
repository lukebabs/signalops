...
LOCAL_PORT="${SIGNALOPS_K8S_MESH2_LOCAL_PORT:-}"

fail() {
  echo "signalops_k8s_mesh2_staging_route_smoke_failed: $*" >&2
  exit 1
}
...
raise SystemExit(1)
PYPORT
)" || fail "no free local mesh smoke port found in 18180-18219"
fi
[[ "$LOCAL_PORT" =~ ^[0-9]+$ ]] || fail "SIGNALOPS_K8S_MESH2_LOCAL_PORT must be numeric"

repo_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$repo_dir"

port_forward_log="$(mktemp -t signalops-k8s-mesh2-port-forward.XXXXXX.log)"
port_forward_pid=""
...
SIGNALOPS_K8S_MESH2_LOCAL_PORT="$LOCAL_PORT" "$PYTHON_BIN" -m pytest -q python/tests/test_k8s_mesh2_signalops_staging_route.py
