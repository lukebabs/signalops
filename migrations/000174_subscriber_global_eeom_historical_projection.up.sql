...
SELECT DISTINCT ON (canonical.global_asset_id, record.session_date, COALESCE(record.payload->>'earnings_event_id', ''))
...
ORDER BY canonical.global_asset_id, record.session_date, COALESCE(record.payload->>'earnings_event_id', ''), record.observed_at DESC, record.global_evidence_id DESC;
