...
- OpenBao is operated with proven backup/restore, seal/unseal, audit, and per-plane policy controls before SignalOps production workloads depend on it; true HA is preferred but not a launch blocker when those controls are current.
...
OpenBao is platform-critical infrastructure for the Kubernetes migration. SignalOps workloads should not be cut over to OpenBao-backed secret injection until the recovery and policy plane has recorded evidence for:

- at least one ready OpenBao server pod reachable through the in-cluster service;
- a documented path to add standby nodes or an equivalent HA topology as resilience hardening;
...
To measure OpenBao HA hardening, run the repository verifier from the SignalOps workspace:
...
Single-node mode requires at least one ready OpenBao server pod and emits `production_cutover_allowed=false` because the verifier is a hardening check, not a production cutover approval. A separate production decision must confirm backup/restore, seal/unseal, audit, policy, and rollback evidence before traffic moves.
...
- OpenBao health, seal state, injector health, audit-log delivery, backup/restore status, and the path to add HA nodes must be observable before production cutover.
