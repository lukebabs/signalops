...
The target production Kubernetes edge uses Gateway API with Istio/Envoy as the K3s ingress authority for node routing. The current base scaffold still includes rollback/reference edge allowances because Docker Traefik is the Compose-era production edge until the named cutover moves public traffic to Istio. The namespace running the active Kubernetes edge gateway must carry the matching ingress-plane label before app ingress can admit traffic:

```yaml
signalops.syncratic.io/ingress-plane: istio  # target K3s ingress authority
```

Traefik is retained as rollback/reference authority only while Docker Compose remains live production authority. ingress-nginx, if present in the cluster, is historical/baseline infrastructure and is not the target SignalOps production ingress controller.
...
2. Continue the Istio/Gateway API cutover path, treating Istio as the K3s ingress authority for staging and future production `signalops.syncratic.io` web, `/v1/*`, `/auth/*`, and Stripe webhook routing.
